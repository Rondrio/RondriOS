...
LCARS::InputField::InputField(int x, int y, int w, int h, Font * font, std::string placeholder) : Component({x, y, w, h}) {

	m_font = font;

	m_content_text		= new Text("", m_font);
	m_content_text->SetColor({255, 255, 255, 255});

	m_placeholder_text	= new Text(placeholder, m_font);
	m_placeholder_text->SetColor({100, 100, 100, 255});

	m_starred_text		= new Text("", m_font);
	m_starred_text->SetColor({255, 255, 255, 255});
	...
	if(FilterInput(ev->text, textlen)) {
		...
		if(m_caret_position == m_content_text->Length()) {
			*m_content_text += ev->text;
			...
			std::string pre		= m_content_text->GetText().substr(0, m_caret_position);
			std::string post	= m_content_text->GetText().substr(m_caret_position);
...
LCARS::Text * LCARS::InputField::GetPlaceholder() {
...
LCARS::Text * LCARS::InputField::GetText() {
...
void LCARS::InputField::Paint(PaintContext * paintctx) {
	...
	if(m_placeholder_text && m_content_text->Length() != 0) {
		paintctx->DrawText(m_padding, m_vpadding, m_shown_text);

	} else {
		paintctx->DrawText(m_padding, m_vpadding, m_placeholder_text);
	}
	...
	if(m_has_kb_focus) {
		...
		std::string substring = m_shown_text->GetText().substr(0, m_caret_position);
		int pixheight, pixlen;
		TTF_SizeText(m_font->GetFont(), substring.c_str(), &pixlen, &pixheight);
		...
		case SDL_KEYDOWN: {
			...
			if(kbe.keysym.sym == SDLK_LEFT) {
				...
				if(m_content_text->Length() > 0) {
					if(m_caret_position == m_content_text->Length()) {
						m_content_text->PopBack(1);
						m_starred_text->PopBack(1);

...
LCARS::Group::Group(int x, int y, int cols, int rows, int width, int height, int col_padding, int row_padding)
	: Component((Rect) {x, y, cols*(width+col_padding), rows*(height+row_padding)})
...
void LCARS::Group::HandleSDLEvent(XEvent * ev) {
	//TODO: Leave empty, I guess.
}

void LCARS::Group::OnUnhandledSDLEvent(XEvent * ev) {
	//TODO: Leave empty, I guess.
}
